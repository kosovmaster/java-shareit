...
public class ItemControllerTest {
    ...
    public void searchItem() {
        ...
        List<ItemDto> itemDtoList = List.of(
                new ItemDto(2L, "hammer", "wooden hammer", true, 1L),
                new ItemDto(3L, "shovel", "sand shovel", true, 2L));

        when(itemService.searchItem(anyString(), anyLong(), anyInt(), anyInt())).thenReturn(itemDtoList);
        mvc.perform(get("/items/search?text=", 10L, 2L)
                ...
                .andExpect(status().isOk());
        verify(itemService).searchItem(anyString(), anyLong(), anyInt(), anyInt());
    }
