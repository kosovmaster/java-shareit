package ru.practicum.shareit.booking.validator.state;
...
public class ValidStateImpl implements ConstraintValidator<ValidState, String> {
    ...
    public boolean isValid(String state, ConstraintValidatorContext constraintValidatorContext) {
        ...
        boolean isState = state.equals("CURRENT")
                || state.equals("PAST")
                || state.equals("FUTURE")
        ...
        if (!isState) {
            ...
            constraintValidatorContext.buildConstraintViolationWithTemplate("Unknown state: " + state).addConstraintViolation();
            return false;
        }
        return true;
    }
